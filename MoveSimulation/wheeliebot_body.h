...
// Header guard
#ifndef __WHEELIEBOT_BODY_H
#define __WHEELIEBOT_BODY_H
...
namespace lpzrobots{
  ...
  /**
   * WheelieBotBody robot: two separated wheel on each side of the body
   * Inherit from OdeRobot
   */
  class WheelieBotBody: public OdeRobot{
    public:
       ...
       * Contrustructor
       */
      WheelieBotBody(const OdeHandle& odeHandle, const OsgHandle& osgHandle,
                   const WheelieBotBodyConf &conf = getDefaultConf(),
                   const std::string& name = "WheelieBot");

      /**
       * Default configuration of the robot
       */
      static WheelieBotBodyConf getDefaultConf(){
        WheelieBotBodyConf conf;
        conf.bodyRadius         = 1.;
        conf.bodyHeight         = .5;
        conf.bodyMass           = 1.;
